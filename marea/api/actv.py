from marea.api.marea import get_istantanea_marea
...
def posting_actv(db_data: BotData):

    hight = get_istantanea_marea()
    actv_data, numb = get_actv(hight)

    try:
        if int(numb) == int(db_data.data["actv_number"]):
            return
    except Exception:
        pass
    db_data.channel.telegram_channel_delete_message(
        db_data.data.get("actv_mex", None)
    )

    db_data.data["actv_h"] = hight

    if actv_data:
        message, flag = db_data.channel.telegram_channel_send(actv_data)
        if flag:
            db_data.data["actv_mex"] = message
    db_data.data["actv_number"] = numb
    db_data.save()
